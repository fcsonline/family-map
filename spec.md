I want to create a React application, using react-flow to draw a family genealogical tree. The idea is that the source of truth is a CSV file, with all the information. Each row is a family member, with unique id, name, birth place, birth day, death day, father, mother, married to. With this information I want to distribute the nodes in this way:
...
Whe should have a search box to be able to find some specific person.
1. Project scaffold
   - Create Vite + React app.
   - Install deps: reactflow, papaparse, react-icons.
2. CSV source of truth
   - Add public/people.csv with headers:
     id,name,birth_day,death_day,birth_place,father,mother,married_to
...
5. React Flow nodes/edges
   ...
   - MarriageNode: small connector.
   - Edges:
     - partner → marriage node.
     - marriage → child.
6. Search UI
   - Search box with case-insensitive name match.
   - Highlight selected node and use fitView / center on it.
   - Optionally dim non-selected nodes for clarity.
7. Styling
   - Clean card UI with shadow, rounded corners, consistent spacing.
   - Color accents for alive/deceased or generation badges.
8. Run + verify
   - Start dev server and validate layout, search, and edges.
